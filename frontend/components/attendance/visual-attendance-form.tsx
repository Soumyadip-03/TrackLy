...
// Add a type for attendance records
...
export function VisualAttendanceForm() {
  ...
  useEffect(() => {
    const loadData = () => {
      ...
      // Load holidays - now supports both old format (string[]) and new format (Holiday[])
      const savedHolidays = getFromLocalStorage<string[] | Holiday[]>('holidays', []);
      
      // Check if holidays are in the new format (objects with date and reason)
    ...
    // Add event listener to refresh the view when schedule changes
  ...
  useEffect(() => {
    ...
    // Check for existing attendance records for this date including auto-present records
    checkExistingAttendance(date);
  }, [date]);
  
  // Check for existing attendance records for this date including auto-present records
  const checkExistingAttendance = (selectedDate: Date) => {
    // Get all records including auto-present ones
  ...
  const handleAllDay = async (allDayStatus: "present" | "absent") => {
    ...
    try {
      ...
      // Create records for each class
      for (const classEntry of nonBreakClasses) {
        // Get the class name from the available fields
        const className = classEntry.name || classEntry.subject || "Unknown Class";
      ...
      dispatchEvent(new Event('attendanceUpdated'));
      ...
      // Show success message with both toast and popup notification
      toast({
        title: "Success",
  ...
  const handleDirectAttendance = async (classId: string, attendanceStatus: "present" | "absent") => {
    ...
    try {
      ...
      // Check if current status is the same as requested (double-click case)
      const isToggle = existingIndex >= 0 && records[existingIndex].status === attendanceStatus;
      
      // Updated records to track changes
      const updatedRecords: Record<string, any> = { ...existingRecords };
      
      if (isToggle) {
        // Toggle case: Remove the attendance record
        if (existingIndex >= 0) {
          records.splice(existingIndex, 1);
        }
        
        // Remove from local state
        const newUpdatedRecords = { ...updatedRecords };
        delete newUpdatedRecords[classId];
        ...
        try {
          await fetch(`${API_URL}/attendance/per-subject`, {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${localStorage.getItem("trackly_token")}`
            },
            body: JSON.stringify({
              date: formattedDate,
              subjectId: classId
          ...
          // Continue since we've updated localStorage
        }
        
        // Update local state
        setExistingRecords(newUpdatedRecords);
        ...
        // Normal case: Add or update attendance
        const className = classEntry.name || classEntry.subject || "Unknown Class";
        const classType = classEntry.classType || "Unknown";
        ...
        // Database sync removed - using localStorage only
